...
class UserManagement(DatabaseSupport):
    ...
    def create_account(self, data):
        ...
        for dictionary in data:
            ...
            if key == 'username':
                ...
                if new_key in db_users['users']:
                    new_account_dict_nok_user = server_response.E_ACCOUNT_EXIST
                    new_account_json_nok_user = json.dumps(new_account_dict_nok_user)
                    return new_account_json_nok_user
            elif key == "permissions":
                if value not in ['user', 'admin']:
                    new_account_dict_nok_perm = server_response.E_WRONG_PERMISSIONS
        ...
        if len(new_key) > 0:
            ...
            new_account_dict_ok = server_response.NEW_ACCOUNT_CREATED
            new_account_json_ok = json.dumps(new_account_dict_ok)
            return new_account_json_ok
        else:
            new_account_dict_nok = server_response.E_USER_NAME_NOT_PROVIDED
    ...
    def user_list(self):
        ...
        user_show_dict = {server_response.EXISTING_ACCOUNTS: exist_users}
    ...
    def user_info(self, username):
        ...
        if username not in db_users['users']:
            user_to_check_dict_nok = server_response.E_USER_DOES_NOT_EXIST
            ...
            user_info_dict = {server_response.ACCOUNT_INFO: exist_user}
    ...
    def user_del(self, user_to_del):
        ...
        if user_to_del not in db_users['users']:
            user_del_dict_nok = server_response.E_USER_DOES_NOT_EXIST
            user_del_json_nok = json.dumps(user_del_dict_nok)
            return user_del_json_nok
        elif user_to_del in db_users['logged_users']:
            user_del_dict_logged = server_response.E_USER_LOGGED_CANNOT_BE_DELETED
            ...
            self.database_support.save_db_json(db_users, server_data.USERS_DATABASE)
            self.database_support.save_db_json(db_msgs, server_data.MESSAGES_DATABASE)
            user_del_dict = {user_to_del: server_response.USER_DELETED}
    ...
    def user_perm(self, data):
        ...
        if user_to_change_permission not in db_users['users']:
            user_perm_dict_nok = server_response.E_USER_DOES_NOT_EXIST
            ...
            user_perm_dict_logged = server_response.E_USER_LOGGED_CANNOT_CHANGE_PERMISSIONS
            user_perm_json_logged = json.dumps(user_perm_dict_logged)
            return user_perm_json_logged
        elif new_permissions not in ['user', 'admin']:
            user_perm_dict_wrong = server_response.E_WRONG_PERMISSIONS
            ...
            self.database_support.save_db_json(db_users, server_data.USERS_DATABASE)
            user_perm_dict = {user_to_change_permission: server_response.USER_PERMISSIONS_CHANGED}
    ...
    def user_stat(self, data):
        ...
        if user_to_change_status not in db_users['users']:
            user_stat_dict_nok = server_response.E_USER_DOES_NOT_EXIST
            ...
            user_stat_dict_logged = server_response.E_USER_LOGGED_CANNOT_CHANGE_STATUS
            user_stat_json_logged = json.dumps(user_stat_dict_logged)
            return user_stat_json_logged
        elif new_status not in ['banned', 'active']:
            user_stat_dict_wrong = server_response.E_WRONG_STATUS
            ...
            self.database_support.save_db_json(db_users, server_data.USERS_DATABASE)
            user_stat_dict = {user_to_change_status: server_response.USER_STATUS_CHANGED}
