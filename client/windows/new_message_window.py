...
class NewMessageWindow(BaseWindow):
    def __init__(self, stdscr, middle_window, login_window, console):
        ...
        self.window = self.stdscr.subwin(cln_datas.NEW_MSG_HEIGHT, cln_datas.NEW_MSG_WIDTH, self.maxY // 4,
                                         self.maxX // 4)
    ...
    def init_window(self):
        ...
        self.window.refresh()
        self.window.addstr(10, cln_datas.NEW_MSG_WIDTH - 9, f'0/250 ')
        self.window.refresh()
    ...
    def number_of_chars(self):
        ...
        if count >= 250:
            ...
            self.window.attron(curses.color_pair(cln_datas.COLOR_PAIR))
        self.window.addstr(10, cln_datas.NEW_MSG_WIDTH - 9, f'{count}/250 ')
        self.window.refresh()
    ...
    def get_new_message(self):
        ...
        curses.echo()
        self.window.refresh()
        self.window.addstr(1, 12, " " * (cln_datas.ADDUSER_WIDTH - 24))
        self.window.addstr(2, 12, " " * (cln_datas.ADDUSER_WIDTH - 24))
        ...
        while True:
            ...
            if key == 10:  # Enter
                ...
                if content_x > 15:
                    ...
                    self.window.insstr(content_y, cln_datas.NEW_MSG_WIDTH - 2, " ")
                    if content_x < cln_datas.NEW_MSG_WIDTH - 2:
                        self.window.move(content_y, content_x)
                    self.message_exceeded = False
                    ...
                    if content_y > 2:
                        content_y -= 1
                        content_x = len(self.content) % (cln_datas.NEW_MSG_WIDTH - 15) + 15
                        self.window.move(content_y, content_x)
                        self.message_exceeded = False
                ...
                char = chr(key)
                if char.isprintable() and len(self.content) < cln_datas.MAX_MESSAGE_LENGTH:
                    if content_x == cln_datas.NEW_MSG_WIDTH - 3:
                        if content_y < cln_datas.NEW_MSG_HEIGHT - 1:
                            content_y += 1
                            content_x = 15
                    self.content += char
                    if len(self.content) >= cln_datas.MAX_MESSAGE_LENGTH:
                        self.window.move(content_y, content_x)
